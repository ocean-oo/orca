...
- [x] Add error states for unavailable source hosts, missing provider auth, unsupported provider on host, and version-skewed remote servers.
...
- [x] Migrate Automation.projectId away from repo-id semantics toward logical project id plus explicit run host/project host setup.
...
- [x] Electron-test Tasks source picker/display and task drawer mutations.
...
- [x] 2026-06-13: Completed Tasks source picker/display and drawer mutation verification by combining prior Electron source-display coverage with source-boundary regression tests for GitHub, GitLab, Linear, and Jira drawers. The mutation tests assert provider actions reuse the drawer item's `TaskSourceContext`/repo selector and source-scoped optimistic patches; live mutation against external providers was intentionally not performed to avoid modifying real issues. Verified with 5 focused task/source tests, targeted oxlint, full `pnpm run typecheck`, and `git diff --check`.
- [x] 2026-06-13: Completed GitLab path-only selector verification. GitLab Tasks/list/detail/mutation paths now carry `repoId`/`TaskSourceContext`, desktop IPC validates repo/source-host mismatches, runtime/RPC tests route selectors as `id:<repoId>` and preserve `projectRef`, and web preload now has explicit repo-id selector coverage instead of only path fallback coverage. Verified with GitLab IPC/runtime/source-boundary/web-preload tests, targeted oxlint, full `pnpm run typecheck`, and `git diff --check`.
